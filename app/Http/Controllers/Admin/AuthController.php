...
class AuthController extends Controller
{
    ...
    public function getUserUsingShopID($shop_id)
    {
        $shop = Shop::find($shop_id);
        if($shop)
        {
            $user = User::find($shop->created_by);
            if($user)
            {
                return response()->json(['user' =>$user]);
            } else {
                return response()->json(['msg' => "User not found or delete user"]);
            }
        } else {
            return response()->json(['msg' => "Shop not found Or delete shop"]);
        }
    }
    ...
    public function cardDetailUpdate($id,Request $req)
    {
        ...
        if($card_data)
        {
            ...
            return response([
                'status' => 0,
                "message" => "Please make some changes first."
            ]);
        }
    }
