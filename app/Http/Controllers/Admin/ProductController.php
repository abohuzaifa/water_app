...
class ProductController extends Controller
{
    ...
    public function createProduct(Request $request)
    {
        ...
        if($category)
        {
            ...
            if(isset($request->galary_images) && count($request->galary_images) > 0)
            {
                foreach($request->galary_images as $img)
                {
                    $images[] = $img;
                }
            }
    ...
    public function productList($id, Request $req){
        $attrs = $req->validate([
        ...
        if($shop)
        {
            ...
            {
                $product = DB::select("SELECT * FROM products WHERE status=1 AND shop_id =".$id." AND category_id = ".$req->category_id);
                if(count($product) > 0){
    ...
    public function galary_images(Request $req)
    {
        $attrs = $req->validate([
            'string' => 'required'
        ]);

        $images = Galary::where('product_name', 'like', '%' . $attrs['string'] . '%')->pluck('images');
        $galary = [];
        foreach($images as $img)
        {
            $array = json_decode($img, true);
            foreach($array as $arr)
            {
                $galary[] = $arr;
            }
        }
        // echo '<pre>';print_r($galary); exit;
        return response([
            'status' => 1,
            'galary' => $galary,
            'image_base_url' => asset('images/')
